...
class Experiment:
    ...
    def run(self, num_HVs, num_AVs, training, testing, action_min, action_max, Graph):
        ...
        from GRL_Library.agent.Continuous import PPO_agent
        ...
        if Graph:
            from GRL_Net.Model_Continuous.PPO import Graph_Actor_Model, Graph_Critic_Model
            GRL_actor = Graph_Actor_Model(N, F, A, lr, action_min, action_max)
            GRL_critic = Graph_Critic_Model(N, F, A, lr, action_min, action_max)
        else:
            from GRL_Net.Model_Continuous.PPO import NonGraph_Actor_Model, \
                NonGraph_Critic_Model
            GRL_actor = NonGraph_Actor_Model(N, F, A, lr, action_min, action_max)
            GRL_critic = NonGraph_Critic_Model(N, F, A, lr, action_min, action_max)
        ...
        GRL_PPO = PPO_agent.PPO(
            ...
            n_epochs=3,  # update times for one batch_size
            update_interval=100,  # update interval
            model_name="DQN_model"  # model
        )
