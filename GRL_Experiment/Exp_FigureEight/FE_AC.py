...
class Experiment:
    ...
    def run(self, num_HVs, num_AVs, training, testing, action_min, action_max, Graph):
        ...
        actor_optimizer = torch.optim.Adam(actor.parameters(), eps=0.0001)
        critic_optimizer = torch.optim.Adam(critic.parameters(), eps=0.0001)
        # Discount factor
        gamma = 0.9

        # Initialize GRL agent
        GRL_AC = AC_agent_nonshare.AC(
            actor,
            actor_optimizer,
            critic,
            critic_optimizer,
        ...
        save_dir = '../GRL_TrainedModels/AC/NOG5'
        ...
        # Testing
        test_episodes = 10
        load_dir = '../GRL_TrainedModels/AC/NOG5'
        debug_testing = False
        if testing:
