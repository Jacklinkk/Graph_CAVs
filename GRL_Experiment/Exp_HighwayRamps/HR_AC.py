...
class Experiment:
    ...
    def run(self, num_HVs, num_AVs, training, testing, Graph):
        ...
        optimizer = torch.optim.Adam(GRL_Net.parameters(), eps=0.00001)
        # Discount factor
        gamma = 0.9
        ...
        save_dir = '../GRL_TrainedModels/AC/DQN5'
        debug_training = False
        if training:
            Training_GRLModels(GRL_Net, GRL_AC, self.env, n_episodes, max_episode_len, save_dir, debug_training)

        # Testing
        test_episodes = 10
        load_dir = '../GRL_TrainedModels/AC/DQN5'
        debug_testing = False
        if testing:
            Testing_GRLModels(GRL_Net, GRL_AC, self.env, test_episodes, load_dir, debug_testing)
