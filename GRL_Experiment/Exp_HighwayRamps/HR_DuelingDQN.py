...
class Experiment:
    ...
    def run(self, num_HVs, num_AVs, training, testing, Graph):
        ...
        if Graph:
            from GRL_Net.Model_Discrete.Q_Dueling import Graph_Model
            GRL_Net = Graph_Model(N, F, A)
        else:
            from GRL_Net.Model_Discrete.Q_Dueling import NonGraph_Model
            GRL_Net = NonGraph_Model(N, F, A)

        # Initialize optimizer
        optimizer = torch.optim.Adam(GRL_Net.parameters(), eps=0.0005)  # 需要定义学习率
        # Replay_buffer
