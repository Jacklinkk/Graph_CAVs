...
class REINFORCE(object):
    ...
    def choose_action(self, observation):
        ...
        """
        # Action generation
        mu, sigma = self.model(observation)
        sigma = torch.exp(sigma)
        action_probabilities = torch.distributions.Normal(mu, sigma)
        action = action_probabilities.sample()
        log_probabilities = action_probabilities.log_prob(action)
        self.action_memory.append(log_probabilities)
        action = torch.clamp(action, min=self.model.action_min, max=self.model.action_max)
